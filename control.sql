...
CREATE TABLE IF NOT EXISTS config_file (
    ...
    name_source VARCHAR(255) NOT NULL UNIQUE COMMENT 'Tên mô tả của nền tảng âm nhạc',
    source_web VARCHAR(255) NOT NULL UNIQUE COMMENT 'URL nguồn hoặc đường dẫn đến trang web của tảng âm nhạc',
    source_folder_location VARCHAR(255) NOT NULL COMMENT 'Đường dẫn thư mục chứa tệp được crawl từ tảng âm nhạc',
    ...
    dest_table_dm VARCHAR(255) NOT NULL COMMENT 'Tên bảng đích trong kho dữ liệu (Data Mart)'
);
...
CREATE TABLE IF NOT EXISTS log_file (
    ...
    status_log ENUM(
        ...
        'Load_Failed',  	        -- Khi tải dữ liệu vào DW lỗi
        'Extract_DataMart_Start',   -- Khi bắt đầu tải dữ liệu vào DM
        'Extract_DataMart_Success', -- Khi Khi tải dữ liệu vào DM hoàn thành
        'Extract_DataMart_Failed'   -- Khi Khi tải dữ liệu vào DM lỗi
    ) NOT NULL,
